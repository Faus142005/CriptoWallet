...
public class ClasePrincipal {
	...
	private static void crearMoneda() throws SQLException {
		...
		do {
			...
			switch (opcion) {
			case 1:
				GestorDAOs.crearMonedaFiduciaria();
				break;

			case 2:
				GestorDAOs.crearCriptomoneda();
				break;
			}
		} while (opcion != 0);
	}
	...
	private static void listarMonedas() throws SQLException {
		...
		switch (opcion) {
		case 1:
			imprimirListaMoneda(GestorDAOs.listarMonedasPrecioEnDolar());
			esperarTerminal();
			break;

		case 2:
			imprimirListaMoneda(GestorDAOs.listarMonedasNomenclatura());
			esperarTerminal();
			break;
		default:
	...
	private static void imprimirListaStock(List <Stock> listaDeStock) {
		...
		for (Stock stock : listaDeStock) {
			...
			System.out.print(" | Cantidad disponible: "+stock.getCantidadActual());
		}
	}
	...
	private static void listarStock() throws SQLException {
		...
		switch (opcion) {
		case 1:
			imprimirListaStock(GestorDAOs.listarStockCantidad());
			esperarTerminal();
			break;

		case 2:
			imprimirListaStock(GestorDAOs.listarStockNomenclatura());
			esperarTerminal();
			break;
		default:
			System.out.println("Opción inválida, abrazo");
	...
	private static void imprimirListaActivos(List <Activo> listaDeActivo) {
		...
		for (Activo activo : listaDeActivo) {
			...
			System.out.print(" | Cantidad disponible: "+activo.getCantidad());
		}
	}
	...
	private static void listarActivos() throws SQLException {
		...
		switch (opcion) {
		case 1:
			imprimirListaActivos(GestorDAOs.listarActivoCantidad());
			esperarTerminal();
			break;

		case 2:
			imprimirListaActivos(GestorDAOs.listarActivoNomenclatura());
			esperarTerminal();
			break;
		default:
			System.out.println("Opción inválida, abrazo");
