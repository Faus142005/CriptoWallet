...
public class CompraCriptomonedaVista {
	...
	private JLabel etiquetaPrecioCriptoADolar = new JLabel("");
		...
		this.selectorCriptomonedas.setRenderer(new DefaultListCellRenderer() {
			...
			public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
				...
				if (value != null) {
					...
					// Redondeamos el valor en dolar a diez decimales
					String valorRedondeado = String.format("%.10f", c.getValorDolar());
					label.setText(c.getNomenclatura() + " | " + valorRedondeado);
				} else {
		...
		this.selectorFIAT.setRenderer(new DefaultListCellRenderer() {
			...
			public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
				...
				if (value != null) {
					...
					// Redondeamos el valor en dolar a diez decimales
					String valorRedondeado = String.format("%.10f", f.getValorDolar());
					label.setText(f.getNomenclatura() + " | " + valorRedondeado);
				} else {
	...
	public void nuevasCriptomonedas() {
		...
		for (Criptomoneda c : criptomonedas) {
			...
			if (c.getIdMoneda() == GestorDeDatosDeLaAplicacion.getCriptomonedaSeleccionada().getIdMoneda())
				selectorCriptomonedas.setSelectedItem(c);
		...
		Criptomoneda c = (Criptomoneda) selectorCriptomonedas.getSelectedItem();
		// Redondeamos el valor en dolar a diez decimales
		String valorRedondeado = String.format("%.10f", c.getValorDolar());
		etiquetaPrecioCriptoADolar.setText(c.getNomenclatura() + " | " + valorRedondeado + "USD");
	}
	...
	public void nuevasFIAT() {
		...
		HashMap<String, ImageIcon> auxImagenes = FuncionesRecursosPrograma.obtenerIconosDeFIATS();

		for (FIAT f : fiats) {
			selectorFIAT.addItem(f);
			if (f.getIdMoneda() == GestorDeDatosDeLaAplicacion.getFiatSeleccionada().getIdMoneda())
				selectorFIAT.setSelectedItem(f);
			iconosFIATEscaladas.put(f.getNomenclatura(),
					CalculosGenerales.escalarImagenAlto(auxImagenes.get(f.getNomenclatura()), 20));
		}

		FIAT f = (FIAT) selectorFIAT.getSelectedItem();
		// Redondeamos el valor en dolar a diez decimales
		String valorRedondeado = String.format("%.10f", f.getValorDolar());
		etiquetaPrecioCriptoAFIAT.setText(f.getNomenclatura() + " | " + valorRedondeado + "USD");
	}
