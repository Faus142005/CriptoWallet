...
public class TransaccionDAOJDBC implements TransaccionDAO<Transaccion> {
	...
	public int insertarTransaccion(Transaccion t) throws SQLException {
		...
		try (Connection connection = getConnection();
			...
			pstmt.setTimestamp(2, t.getFecha());
		...
		try (Connection connection = getConnection(); PreparedStatement pstmt = connection.prepareStatement(sql);) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				...
				while (rs.next()) {
					...
					Persona persona = new Persona(rs.getInt("ID_PERSONA"), rs.getString("NOMBRES"),
							rs.getString("APELLIDOS"));

					// Crear un objeto Usuario a partir de los datos de la base de datos
					Usuario usuario = new Usuario(rs.getInt("ID_USUARIO"), persona, rs.getString("EMAIL"),
							rs.getString("PASSWORD"), rs.getBoolean("ACEPTA_TERMINOS"));

					// Crear un objeto Transaccion a partir de los datos de la base de datos
					Transaccion transaccion = new Transaccion(rs.getInt("ID_TRANSACCION"), rs.getString("RESUMEN"),
							rs.getTimestamp("FECHA_HORA"), usuario);
	...
	public Transaccion buscarTransaccion(int idTransaccion) throws SQLException {
		...
		try (Connection connection = getConnection(); PreparedStatement pstmt = connection.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				...
				if (rs.next()) {
					// Creo Persona
					Persona persona = new Persona(rs.getInt("ID_PERSONA"), // ID de la persona
							rs.getString("NOMBRES"), rs.getString("APELLIDOS"));

					// Creo y devuelvo Usuario
					Usuario usuario = new Usuario(rs.getInt("ID_USUARIO"), // ID del usuario
							persona, // Objeto Persona asociado
							rs.getString("EMAIL"), rs.getString("PASSWORD"), rs.getBoolean("ACEPTA_TERMINOS"));

					return new Transaccion(rs.getInt("ID"), // ID de la transaccion
							rs.getString("RESUMEN"), rs.getTimestamp("FECHA_HORA"), usuario);
				}
			}
